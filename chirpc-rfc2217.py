...
# Remote serial port with RFC 2217 protocol, eg:
# Run server RFC 2217 server (where physical serial port exist) with netcat and sredird:
#
# $ while true; do nc -vv -l -p 7000 -s 0.0.0.0 -c "sredird debug /dev/ttyUSB0 /var/lock/sredir"; done
# // open access to serial port /dev/ttyUSB0 via RFC 2217 on TCP port 7000
# // ps. make sure you are in a dialout group, run:
# // $ sudo addgroup "$USERNAME" dialout (need to log out and back in for it to take effect.)

# run chirpc-rfc2217.py on remote device:
# $ python2.7 chirpc-rfc2217.py -r Baofeng_UV-5R --serial rfc2217://192.168.0.100:7000?ign_set_control --mmap=uv5r.mmap --download-mmap

# this command download mmap from Baofeng UV-5R connected to the serial port /dev/ttyUSB0 on machine ip 192.168.0.100
...
#     logger.handle_options(options)
    ...
    if options.serial == "mmap":
        ...
        s = serial.serial_for_url(options.serial)
